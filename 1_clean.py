...
for drug in name_dict:
    ...
    name_finds = (temp_frame.loc[temp_frame['prescription'].str.contains(drug), 'prescription'].apply(lambda x: findName(x))) # find rows with drug (exact word match)
    name_indices = name_finds[name_finds==1]
    drug_count -= 1
    if len(name_indices)>0:
        temp_frame.loc[name_indices.index, 'prescription_new'] = temp_frame.loc[name_indices.index, 'prescription'].str.replace(drug, name_dict[drug], regex=False) # replace the drug
        temp_frame.loc[name_indices.index, 'prescription_new'] = temp_frame.loc[name_indices.index, 'prescription_new'].str.replace('ee', 'e') # in some subsitutions, a drug name gets replaced by a drug name that ends with an e (dicycloverin --> dicycloverine); this leads to an additional 'e' after subsitution

# attach temporary frame as new prescription column
meds['prescription_new'] = temp_frame['prescription_new']
...
meds.loc[meds['prescription']=='NULL', 'prescription'] = meds['prescription_old']

# based on skimming through the data frame, some entries have to be manually altered
meds.loc[meds['prescription'].str.contains('patch'), 'prescription'] = meds.loc[meds['prescription'].str.contains('patch'), 'prescription'].str.replace('hyoscine', 'hyoscine hydrobromide')
meds.loc[meds['prescription'].str.contains('300'), 'prescription'] = meds.loc[meds['prescription'].str.contains('300'), 'prescription'].str.replace('hyoscine', 'hyoscine hydrobromide')
meds.loc[meds['prescription'].str.contains('400'), 'prescription'] = meds.loc[meds['prescription'].str.contains('400'), 'prescription'].str.replace('hyoscine', 'hyoscine hydrobromide')
meds.loc[meds['prescription'].str.contains('600'), 'prescription'] = meds.loc[meds['prescription'].str.contains('600'), 'prescription'].str.replace('hyoscine', 'hyoscine hydrobromide')
...
# remove rows with blank date column
meds = meds.loc[~meds['date'].isna(), :]
#remove unnecessary columns
meds.drop(['read_code','bnf','dmd','meds_read_len','prescription_read'], axis=1, inplace=True)
